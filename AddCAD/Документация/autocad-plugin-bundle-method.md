...
PNG формально допустим (menuGEO их использует), но с ним панель ленты
...
1. **Нет `Menu_Package_Info.xml`** — самая частая причина, см. выше.
   ...
   (у MDSTools 349 КБ). Шесть байт нормальны только для адаптации вообще
...
Второй живой пример для сверки схемы — `MDSTools.bundle` там же.
