...
### Иконки: PNG, и не только внутри CUIX

Работают оба формата, но **брать надо PNG**: прозрачные пиксели BMP лента
рисует чёрным, и вокруг иконки появляется чёрная плашка. PNG с альфа-каналом
показывается правильно. Оба пишутся из Python без сторонних библиотек:
PNG — через `zlib.compress` и три чанка IHDR/IDAT/IEND, BMP — через
`struct.pack` (`<2sIHHI` для файлового заголовка, `<IiiHHIIiiII` для
BITMAPINFOHEADER, строки снизу вверх, каналы BGRA).

Объявить расширение в `[Content_Types].xml`:
`<Default Extension="png" ContentType="image/png" />`.

**Где картинка ищется — зависит от того, кто её просит.** Это стоило часа
разбирательств, поэтому по пунктам, всё проверено на AutoCAD 2025:

| Кто | Как задаётся | Откуда берётся файл |
|---|---|---|
| `MenuMacro` (значки кнопок и пунктов списка) | `<SmallImage Name="x_16.png" />` дочерним элементом | из самого CUIX |
| `RibbonSplitButton` (своя картинка кнопки-списка) | `SmallImage="x_16.png"` атрибутом | **файл на диске**, по путям поиска |

То есть картинку кнопки-списка мало положить внутрь CUIX — оттуда она
не читается ни атрибутом, ни дочерним элементом, на кнопке вылезает облако
с вопросительным знаком. Её кладут **файлом** рядом и добавляют папку
в пути поиска вспомогательных файлов.

И тут вторая ловушка: `AppDescription="Support Path"` в `PackageContents.xml`
в этот список **не попадает**. Проверяется одной строкой —
`(getvar "ACADPREFIX")` показывает пути поиска, а `(findfile "x.png")`
вернёт `nil`. Папок плагинов из `ApplicationPlugins` там нет вообще, ни своих,
ни чужих. Поэтому путь дописывает установщик — в значение `ACAD` ветки
`...\Profiles\<профиль>\General`, и убирает его при удалении.

Убрать картинку у кнопки-списка совсем — не выход: на значок первого пункта
списка AutoCAD не переключается, кнопка остаётся вовсе без иконки.

### Шрифты: не полагаться на чужие

Отдельная ловушка, которая всплывает только на другой машине. Если создавать
текстовый стиль под шрифт вроде `GOST_Common Italic.ttf`, поставленный вместе
с какой-нибудь надстройкой, то там, где надстройки нет, AutoCAD молча
подставит свой шрифт — надписи поедут по виду и по ширине.

Правило: перебирать шрифты по списку и брать первый доступный, начиная
с желаемого и заканчивая тем, что есть всегда (`simplex.shx`). Из TTF
у AutoCAD всегда есть `isocpeur.ttf` — по начертанию это ближайшая замена
шрифту по ГОСТ 2.304.

Проверять наличие: шрифты AutoCAD лежат в его папке `fonts`, она в путях
поиска, и находятся обычным `(findfile "isocpeur.ttf")`. Шрифты Windows
в путях поиска не значатся, их проверять полным путём:
`(findfile (strcat (getenv "WINDIR") "\\Fonts\\<файл>"))`.

И имя стиля брать своё, а не общеупотребительное: в чужом чертеже стиль
с именем «ГОСТ» может оказаться заведён под что угодно, и текст унаследует
его вид.
